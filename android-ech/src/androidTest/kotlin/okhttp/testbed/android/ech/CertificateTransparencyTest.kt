 ...
 * security config opts one name out and explicitly enforces CT for the other. Explicit opt-in is
 * required because the fixture uses a custom trust anchor, which Android otherwise exempts from
 * CT even on API 37. Consequently the opt-out request is a control for every ordinary cause of a
 * TLS failure (the CA, validity, hostname, protocol, and route), while the enforced request must
 * fail specifically at the CT policy check. This avoids relying on
 * `no-sct.badssl.com`, whose certificate can expire or whose behavior can change independently of
 * this repository.
 *
 * CT is available from API 36. API 37 remains independently important: that is the current
 * platform implementation and log list OkHttp users encounter, and it runs on every change.
 */
class CertificateTransparencyTest {
  ...
  fun unloggedCertificateConnectsWhenCtIsOptedOut() {
    get(CT_OPT_OUT_NAME).use { response ->
      AndroidTlsPolicyReport.recordOptOut(true, "HTTP ${response.code}")
  ...
  fun unloggedCertificateIsRejectedWhenCtIsEnforced() {
    ...
    val messages =
      generateSequence<Throwable>(failure) { it.cause }
        ...
        .joinToString("\n")
    AndroidTlsPolicyReport.recordEnforced(false, "${failure.javaClass.simpleName}: $messages")
